...
__global__ void sgemm_vectorized_kernel(int M, int N, int K,
    ...
    for (int tileK = 0; tileK < K; tileK += BK) {
        ...
        // Collaborative load of B tile to shared memory (with vectorized access when aligned)
...
// Wrapper function
inline void run_sgemm_vectorized(int M, int N, int K,
    ...
    sgemm_vectorized_kernel<BM, BN, BK, TM, TN><<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
}
